...
class SalaryController extends Controller
{
    ...
    public function getEmployee(Request $request)
    {
        ...
        foreach ($data as $key => $attend) {
            ...
            $html[$key]['tdsource'] .= '<td>' . '<input type="hidden" name="employee_id[]" value="' . $attend
            ->employee_id . '">' . '<input type="checkbox" name="checkmanage[]" value="' . $key . '"' . $checked . ' style="transform:scale(1.5);margin-left:10px;">' . '</td>';
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty(@$data) || empty($checkdata)) {
            return redirect()->route('accounts.salary.view')->with('success', 'Student Fee Updated Successfully');
        } else {
